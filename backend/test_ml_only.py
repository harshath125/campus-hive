...
print("=" * 65)
print("CAMPUS HIVE ML VIBE MATCHER - CLUSTER ACCURACY")
...
for cname, (rng, email) in CLUSTERS.items():
    ...
    sep = ai - ao
    t1 = matches[0].get("user", {}).get("email", "") in own
    t3 = sum(1 for m in matches[:3] if m.get("user", {}).get("email") in own)
    results[cname] = {"in_avg": ai, "out_avg": ao, "sep": sep, "top1": t1, "top3": t3, "ok": sep > 0}

print()
hdr = "%-10s | %7s | %7s | %7s | %5s | %5s" % ("Cluster", "In-Avg", "Out-Avg", "Sep", "Top-1", "Top-3")
print(hdr)
print("-" * len(hdr))
for cname, s in results.items():
    print("%-10s | %5.1f%% | %5.1f%%  | %+5.1f%% | %5s | %d/3 %s" % (
        cname, s["in_avg"], s["out_avg"], s["sep"],
        "YES" if s["top1"] else "no", s["top3"],
        "[OK]" if s["ok"] else ""))

correct = sum(1 for s in results.values() if s["ok"])
total = len(results)
print()
print("Accuracy: %d/%d clusters (%d%%)" % (correct, total, (correct * 100 // total) if total else 0))

# Top 5 for AI/ML user
print()
print("=" * 65)
print("ARJUN REDDY (AI/ML) - TOP 10 MATCHES")
print("=" * 65)
data1 = get_matches("student01@campushive.com")
for i, m in enumerate(data1.get("matches", [])[:10]):
    u = m.get("user", {})
    print("  #%d: %-20s %5.1f%% | %s %s Y%s | %s" % (
        i + 1, u.get("name", "?"), m.get("score", 0),
        u.get("branch", ""), u.get("section", ""),
        u.get("year", ""),
        m.get("common_tags", [])))

# Top 5 for Web Dev user
print()
print("=" * 65)
print("DIVYA MENON (Web Dev) - TOP 10 MATCHES")
print("=" * 65)
data2 = get_matches("student08@campushive.com")
for i, m in enumerate(data2.get("matches", [])[:10]):
    u = m.get("user", {})
    print("  #%d: %-20s %5.1f%% | %s %s Y%s | %s" % (
        i + 1, u.get("name", "?"), m.get("score", 0),
        u.get("branch", ""), u.get("section", ""),
